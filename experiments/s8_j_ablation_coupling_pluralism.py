...
# -*- coding: utf-8 -*-
"""
S8 — J-Ablation / Coupling Pluralism Test
=========================================

Artifact test for S0-S7.

The problem:
    If J(x,y)=exp(-distance) is already metric-like, then proto-geometry may
    be imported rather than emergent.

S8 tests multiple coupling functions J under the same dynamics:

    metric_exp           original metric-like control
    cosine_resonance     periodic resonance coupling
    mixed_resonance      non-monotonic mixed resonance
    threshold_resonance  hard resonance gate
    random_projection    deterministic random-feature coupling

If several non-metric J-types still produce stable populations, persistent
tracks, coupled identity networks, and finite geodesic structure, then the
recoherence mechanism is more robust than the original metric-like J.
...
def save_records_csv(records, output_dir="s8_outputs", filename="s8_j_ablation_results.csv"):
...
def plot_summary_by_j(records, output_dir="s8_outputs", filename="s8_j_ablation_summary.png"):
    output_path = ensure_output_dir(output_dir) / filename
    ...
    plt.title("S8 J-ablation summary")
    plt.legend()
...
def run_s8_ablation(runs=50, j_names=None, output_dir="s8_outputs", cluster_threshold=0.6, coupling_threshold=0.25):
    if j_names is None:
        j_names = list(J_FUNCTIONS.keys())

    print("=== S8 J-ablation / coupling pluralism test ===")
...
If only metric_exp produces stable geometry:
    The result is likely a metric artifact.

If several non-metric J-functions also produce stable populations, stable
tracks, coupled identity networks, nontrivial components, and finite geodesics:
    The recoherence mechanism is more robust than the original metric-like J.

Output files:
    s8_outputs/s8_j_ablation_results.csv
    s8_outputs/s8_j_ablation_summary.png
"""
